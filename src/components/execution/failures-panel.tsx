          ...
          <div className="flex-1 h-6 bg-zinc-800/50 rounded overflow-hidden">
            <motion.div
              ...
              className={cn(
                ...
                agg.severity === 'high' ? 'bg-orange-500/70' :
                agg.severity === 'medium' ? 'bg-amber-500/70' :
                'bg-slate-500/70'
              )}
            >
        ...
        <div className="flex-1 min-w-0">
          ...
          {record.recoveryAction && (
            <div className="flex items-start gap-1.5 mt-2 pt-2 border-t border-zinc-800/40">
              <RecoveryIcon className={cn('w-3 h-3 mt-0.5 shrink-0', record.recoveryStatus === 'success' ? 'text-emerald-500' : record.recoveryStatus === 'pending' ? 'text-amber-500' : record.recoveryStatus === 'failed' ? 'text-red-500' : 'text-zinc-500')} />
              <div>
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="rounded-lg border bg-zinc-900/50 border-zinc-800/60 p-3">
          <p className="text-lg font-bold text-amber-400 tabular-nums">{recurringFailures.length}</p>
