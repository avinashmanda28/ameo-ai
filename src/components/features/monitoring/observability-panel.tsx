...
const SEVERITY_SCORE_COLORS: Record<string, string> = {
  normal: 'text-primary dark:text-primary/80',
  warning: 'text-slate-400',
  degraded: 'text-slate-500',
  critical: 'text-red-400',
};

function scoreColor(score: number): string {
  if (score >= 90) return 'text-primary dark:text-primary/80';
  if (score >= 70) return 'text-slate-400';
  if (score >= 50) return 'text-slate-500';
  return 'text-red-400';
}

function scoreBg(score: number): string {
  if (score >= 90) return 'bg-primary/10 border-primary/30';
  if (score >= 70) return 'bg-slate-500/10 border-slate-800/30';
  if (score >= 50) return 'bg-slate-500/10 border-slate-800/30';
    ...
    <Card className={cn(
      ...
      <CardContent className="p-4">
        ...
        <Badge
          variant="outline"
          className={cn(
            'mt-2 text-[10px] h-5 px-1.5 border',                    severity === 'normal' ? 'bg-primary/20 text-primary/80 border-primary/30' :
            severity === 'warning' ? 'bg-slate-800 text-slate-400 border-slate-700' :
            severity === 'degraded' ? 'bg-slate-800 text-slate-400 border-slate-700' :
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Badge
            ...
            className={cn(
              ...
              event.level === 'warn' ? 'bg-slate-800 text-slate-400 border-slate-700' :
      ...
      <motion.div variants={itemVariants} className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="ghost"
            size="sm"
            className={cn('h-8 text-xs', autoRefresh ? 'text-blue-400' : 'text-zinc-500')}
                ...
                <>
                  ...
                  {graphAnalysis.issues.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-4">
                      <CheckCircle2 className="w-6 h-6 text-blue-500/50 mb-1" />
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <Badge
                            ...
                            className={cn(
                              ...
                              coord.status === 'active' ? 'bg-blue-900/20 text-blue-400 border-blue-800/40' :
            ...
            <CardContent className="px-4 pb-4">
              {loading ? (
                ...
                <div className="flex flex-col items-center py-4">
                  <CheckCircle2 className="w-5 h-5 text-blue-500/40 mb-1" />
                ...
                <ScrollArea className="max-h-48">
                  <div className="space-y-1.5">
                    {conflictRecords.slice(0, 10).map((coord) => (
                      <div key={coord.id} className="rounded-lg border border-slate-700 bg-slate-950/10 px-3 py-2">
                  ...
                  <div className="space-y-1.5">
                    {lockedRecords.slice(0, 10).map((coord) => (
                      <div key={coord.id} className="rounded-lg border border-slate-700 bg-slate-950/10 px-3 py-2">
                        ...
                        <div className="text-[10px] text-slate-400/70 mt-0.5">
