...
export class GroqAdapter implements ProviderAdapter {
  ...
  async execute(params: {
    ...
    if (!content && content !== '') {
      throw {
        type: 'server_error',
        message: 'Groq returned an empty response — no content in choices[0].message.content',
  ...
  private classifyError(err: unknown): RuntimeError {
    ...
    return {
      ...
      message: `Groq connection failed: ${err instanceof Error ? err.message : String(err)}`,
      providerType: this.type,
      retryable: true,
    };
  }
    ...
    const message =
      (errorBody as Record<string, unknown>)?.error
        ? typeof (errorBody as Record<string, unknown>).error === 'string'
          ...
          : String(((errorBody as Record<string, unknown>).error as Record<string, unknown>)?.message ?? 'Unknown error')
    ...
    switch (response.status) {
      ...
      case 422:
        throw {
          ...
          message: `Groq invalid request: ${message}`,
          statusCode: response.status,
          providerType: this.type,
          retryable: false,
        } satisfies RuntimeError;
      ...
      default:
        throw {
          type: 'unknown',
          message: `Groq error (${response.status}): ${message}`,
          statusCode: response.status,
          providerType: this.type,
